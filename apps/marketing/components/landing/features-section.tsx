...
export function FeaturesSection() {
  ...
  const features = [
    { number: "01", title: t('deploy'), description: t('deployDesc'), visual: "deploy" },
    { number: "02", title: t('ai'), description: t('aiDesc'), visual: "ai" },
    { number: "03", title: t('collab'), description: t('collabDesc'), visual: "collab" },
    { number: "04", title: t('security'), description: t('securityDesc'), visual: "security" },
  ];
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="mb-16 lg:mb-24">
          <h2
            className={`text-primary text-4xl lg:text-6xl font-display tracking-tight transition-all duration-700 ${
...
function AIVisual() {
  // LLM logos - tous les modèles supportés
  const llmLogos = [
    { name: "openai", src: "/models/openai.svg" },
    { name: "perplexity", src: "/models/perplexity.svg" },
    { name: "google", src: "/models/google.svg" },
    { name: "anthropic", src: "/models/anthropic.svg" },
    { name: "copilot", src: "/models/copilot.svg" },
    { name: "mistral", src: "/models/mistral.svg" },
  ];

  const radius = 95; // Lignes plus longues

  return (
    <svg viewBox="-20 0 280 220" className="w-52 h-52 md:w-60 md:h-60">
      {/* Pulse rings - en premier (arrière-plan) */}
      <circle cx="100" cy="80" r="30" fill="none" stroke="currentColor" strokeWidth="1" opacity="0">
        <animate attributeName="r" values="25;85" dur="2s" repeatCount="indefinite" />
        <animate attributeName="opacity" values="0.5;0" dur="2s" repeatCount="indefinite" />
      </circle>

      {/* Connection lines - avant les logos mais après les anneaux */}
      {[0, 1, 2, 3, 4, 5].map((i) => {
      ...
      {/* Orbiting LLM logos - après les lignes pour être au-dessus */}
      {[0, 1, 2, 3, 4, 5].map((i) => {
        const angle = (i * 60) * (Math.PI / 180);
        const x = 100 + Math.cos(angle) * radius;
        const y = 80 + Math.sin(angle) * radius;
        const logo = llmLogos[i];

        return (
          <g key={`logo-${i}`} transform={`translate(${x}, ${y})`}>
            {/* Outer circle - rond extérieur */}
      ...
      {/* Central search icon - en dernier (premier plan), plus grand */}
      <g transform="translate(100, 80)">
        <circle r="22" fill="white" stroke="currentColor" strokeWidth="2" />
        {/* Search magnifying glass icon - plus gros */}
    ...
    >
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 py-12 lg:py-20 border-b border-foreground/10">
        ...
        <div className="shrink-0">
          <span className="font-mono text-sm md:text-xl text-primary">{feature.number}</span>
        </div>
        
        {/* Content */}
        <div className="flex-1 grid lg:grid-cols-2 gap-8 items-center">
          <div>
            <h3 className="text-primary text-3xl lg:text-4xl font-display mb-4 group-hover:translate-x-2 transition-transform duration-500">
              {feature.title}
            </h3>

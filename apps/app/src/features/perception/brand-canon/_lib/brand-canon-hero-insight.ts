...
import { getPerceptionAxisLabel } from "@/features/perception/_lib";

export type BrandCanonHeroInsight = {
  brandName: string;
  title: string;
  summary: string;
  microCopy: string;
...
function buildTrendValues(points: PerceptionTrendPoint[]) {
  return points.map((point) => average([point.positioning, point.factual, point.sentiment]));
}

function getMomentumTone(delta: number): BrandCanonHeroInsight["momentumTone"] {
...
function getMomentumLabel(delta: number, locale: string) {
  if (Math.abs(delta) < 3) {
    return translateI18nText("perception-brand-canon", "heroMomentumStable", locale);
  }
...
export function buildBrandCanonHeroInsight(
  ...
  const strongestAxis = radar.length > 0 ? [...radar].sort((left, right) => right.score - left.score)[0] : null;
  const weakestAxis = radar.length > 0 ? [...radar].sort((left, right) => left.score - right.score)[0] : null;
  ...
  const strongestLabel = strongestAxis ? getPerceptionAxisLabel(strongestAxis.axis, locale) : "";
  ...
  return {
    ...
    summary: strongestAxis
      ? translateI18nText("perception-brand-canon", "heroBestAxisSummary", locale, { label: strongestLabel })
      : "",

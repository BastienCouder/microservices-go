...
import { PromptEditorPage } from "./prompt-editor-page";
...
export function PromptsResponsesWorkspace({ apiBaseURL }: PromptsResponsesWorkspaceProps) {
  ...
  const editorPrompt =
    state.promptEditorState?.mode === "edit"
      ? state.prompts.find(
          (item) =>
            (item.sourcePromptId || item.id) === state.promptEditorState?.promptId,
        ) ?? null
      : null;
        ...
        <Tabs value={state.tab} onValueChange={(value) => state.setTab(value as "prompts" | "responses")} className="flex min-h-0 flex-1 flex-col">
          <div className="border-b px-4 py-3">
            <TabsList>
              ...
              <TabsTrigger value="responses">Reponses</TabsTrigger>
            </TabsList>
          </div>

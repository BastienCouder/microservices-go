...
import { promptScheduleLabel, STAGES } from "./utils";
  ...
  return {
    addAutoGeneratedPrompts: () => {
      params.setManualPrompts((current) => [
        ...buildAutoGeneratedPrompts(params.promptAvailableModels, params.availablePersonas, STAGES),
        ...current,
      ]);
    ...
    const targetPromptIds = Array.from(
      new Set(
        params.filteredPromptRows
          .filter((item) => params.selectedPromptIds.includes(item.sourcePromptId || item.id))

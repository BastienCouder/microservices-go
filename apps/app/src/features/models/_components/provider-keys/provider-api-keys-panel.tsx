...
export type ProviderApiKeysPanelTexts = {
  empty: string;
  configured: string;
  missing: string;
  connect: string;
...
type ProviderKeyFormProps = Omit<
  ProviderApiKeyCardProps,
...
function ProviderKeyForm({
  ...
  return (
    <Field className="mt-2 gap-2 border-t pt-4">
      ...
      {texts.fieldHint ? <FieldDescription>{texts.fieldHint}</FieldDescription> : null}
...
export function ProviderApiKeysPanel({
  ...
  const toggleProviderKeyVisibility = useCallback((provider: string) => {
    setVisibleProviders((current) => ({ ...current, [provider]: !current[provider] }));
  }, []);
        ...
        <div className="mt-4 grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-3">
          {requirements.map((requirement) => (
            <ProviderApiKeyCard
              ...
              requirement={requirement}
              draft={drafts[requirement.provider] ?? ""}
              isSaving={pendingProvider === requirement.provider}

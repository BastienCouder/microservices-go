...
export function CrawlPanel({
  ...
  const reviewingDiscoveredPages =
    viewModel.reviewingURLSelection &&
          ...
          <>
            {reviewingDiscoveredPages ? (
              <DiscoveredPagesSelectionView
                ...
                hasLatestContentCrawl={viewModel.hasLatestContentCrawl}
                onQueryChange={setQuery}
                onTogglePage={viewModel.togglePage}
                onToggleAll={viewModel.toggleAllPages}
                onShowLatestContentCrawl={viewModel.showLatestContentCrawl}
              />
            ) : (
              <CrawlerResultsView
                errorLabel={viewModel.error}
                loadingLatest={viewModel.loadingLatest}
                running={viewModel.crawling}
                remainingCount={Math.max(0, (viewModel.crawlResult?.total ?? viewModel.selectedCount) - (viewModel.crawlResult?.finished ?? 0))}
                query={query}

    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="min-w-0">
          <h4 className="text-sm font-semibold">{PERCEPTION_TEXT.topErrors.title}</h4>
        </div>
      ...
      <div className="space-y-3">
        ...
        <Button asChild variant="ghost" size="sm" className="w-full text-xs">
          <Link href="/optimize">{PERCEPTION_TEXT.topErrors.seeMore}</Link>
        </Button>
      </div>
              ...
              <SheetHeader className="border-b border-border/60 p-4 text-left">
                <div className="mb-2 flex flex-wrap gap-1.5">
                  ...
                  {selectedError.detectedInModels.map((model) => (
                    ...
                    >
                      <Image src={getModelIconForName(model)} alt="" width={12} height={12} className="h-3 w-3 shrink-0" aria-hidden="true" />
                      {model}
                    </Badge>
                  ))}
                </div>
      ...
      <div className="mb-2 flex flex-wrap gap-1.5">
        ...
        {error.detectedInModels.slice(0, 2).map((model) => (
          <Badge key={model} variant="outline" className="inline-flex items-center gap-1 font-normal">
            <Image src={getModelIconForName(model)} alt={model} width={16} height={16} className="h-3 w-3 shrink-0" aria-hidden="true" />
            {model}
          </Badge>
        ))}

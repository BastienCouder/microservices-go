...
export function StepAnalysis({ hideBack = false }: StepAnalysisProps) {
    ...
    return (
        <div className="w-full min-h-[calc(100dvh-180px)] space-y-6 rounded-xl border border-border/70 bg-white p-6 text-center shadow-sm sm:min-h-0 sm:p-8">
            <div>
            ...
            <div className="flex items-center justify-between border-t border-border/70 pt-4">
                ...
                <Button onClick={() => router.push("/dashboard")} disabled={progress < 95}>Go to Dashboard</Button>
            </div>
        </div>
    );
}

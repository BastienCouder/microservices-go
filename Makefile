...
run-billing:
	HTTP_ADDR=:8086 BILLING_DB_HOST=localhost BILLING_DB_PORT=5432 BILLING_DB_USER=billsvc BILLING_DB_NAME=billsvc BILLING_DB_SSLMODE=disable BILLING_DB_PASSWORD=billsvc INTERNAL_JWT_SECRET=$${INTERNAL_JWT_SECRET:?INTERNAL_JWT_SECRET is required} INTERNAL_JWT_ISSUER=api-gateway STRIPE_ENABLED=false go run ./services/billing-service/cmd/api
...
run-ia:
	HTTP_ADDR=:8091 GRPC_ADDR=:9091 IA_EXECUTION_MODE=$${IA_EXECUTION_MODE:-mock} IA_PROVIDER_BASE_URL=$${IA_PROVIDER_BASE_URL:-https://openrouter.ai/api/v1} IA_PROVIDER_HTTP_REFERER=$${IA_PROVIDER_HTTP_REFERER:-http://localhost:30004} IA_PROVIDER_APP_NAME=$${IA_PROVIDER_APP_NAME:-microservices-go} IA_PROVIDER_TIMEOUT_MS=5000 INTERNAL_JWT_SECRET=$${INTERNAL_JWT_SECRET:?INTERNAL_JWT_SECRET is required} INTERNAL_JWT_ISSUER=api-gateway GRPC_ALLOW_INSECURE=true go run ./services/ia-service/cmd/api

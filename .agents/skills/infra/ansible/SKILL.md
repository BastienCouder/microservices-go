...
- **Versions (mandatory)**: Always target the **latest stable** versions of system packages and runtimes (no RC/beta). Avoid ambiguous tags like `latest` in Docker images; prefer pinned `major.minor.patch` for images and **stable channels** for OS packages.

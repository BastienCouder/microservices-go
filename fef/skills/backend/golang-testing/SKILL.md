...
- Do not rely on default environment variable values in test targets.
- Tests must set required environment variables explicitly (`t.Setenv`) or expect startup/config loading to fail.
- When running tests via Docker Compose, always run them in prod compose (`docker compose -f docker-compose.yml ...`), not dev compose (`docker-compose.dev.yml`).

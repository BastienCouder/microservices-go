...
- Always use the latest stable version of LangChain/LangGraph packages and related integrations.
- For this skill, verify recent stable releases for `langchain`, `langchain-core`, `langgraph`, and provider packages (for example `langchain-openai`) before pinning.
- Never use prerelease versions (`alpha`, `beta`, `rc`) unless explicitly requested.
- Pin explicit versions in project manifests once validated.
